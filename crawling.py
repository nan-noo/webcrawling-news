...
for dateStart, dateEnd in date:
    ...
    for helper in range(1, 5):
        ...
        for page in range(i, 100+i):  # 50page 단위로 수집
            ...
            start = (page - 1) * 10 + 1
            url = 'https://search.naver.com/search.naver?where=news&sm=tab_pge&query=' + query + '&sort=' + sort + '&ds=' + dateStart + '&de=' + \
                dateEnd + '&nso=so%3Add%2Cp%3Afrom' + \
                dateStart.replace(
                    '.', '') + 'to' + dateEnd.replace('.', '') + '&start={}'.format(start)
            ...
            # 현재 페이지의 뉴스 중 모든 네이버뉴스 a tag만 가져옴
            a_lists = lists.find_all("a", attrs={"class": "info"})
            ...
            for a in a_lists:
                # 네이버 뉴스 링크만 저장
                if a.attrs["href"].startswith("https://news.naver.com"):
                    paths.append(a.attrs["href"])  # 각 네이버뉴스의 path를 저장.

            # page 별로 해당하는 path만 읽어야하므로 => 2 page가 되면 새로운 index부터 읽어야하기 때문
            new_paths = paths[path_len:]
            ...
            for path in new_paths:
                ...
                # 주석 제거: // flash 오류를 우회하기 위한 함수 추가function _flash_removeCallback() {}
                data = data[61:]
                #print('본문 내용 : ', data)
                ...
                # press 추가하기
                try:
                    print(soup.find("a", attrs={"class": "nclicks(atp_press)"}).find(
                        "img").get("title").strip())
                    press.append(soup.find("a", attrs={"class": "nclicks(atp_press)"}).find(
                        "img").get("title").strip())
                except:
                    press.append('')
        ...
        my_dictionary = {"path": paths, "title": titles,
                         "date": dates, "press": press, "content": contents}
        # 전체 데이터를 긁은 list인 total을 dataframe으로 변환시켜주면서 각 column의 이름을 부여
        ...
        data.to_excel("crawling_data/" + query + "/" +
                      datatitle + ".xlsx", engine="xlsxwriter")
