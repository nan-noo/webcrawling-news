...
for dateStart, dateEnd in date:
    ...
    for i in range(1,400,100): ### 100page단위, 400page까지 있음
        ...
        data = pd.read_excel("./crawling_data/" + query + "/" + dataTitle + ".xlsx")
        paths = data["path"]
        titles = data["title"]
        ...
        for path in paths:
            ...
            if total_num_repl is None: 
                ...
                if total_num_repl == 0: # 현재 댓글이 없음
                    ...
                    recomm = [count.text.strip().replace(',', '') for count in cbox_recomms] # 자동접힘 댓글은 못 읽어옴, ''으로 읽어와서 int()시 에러남
                    unrecomm = [count.text.strip().replace(',', '') for count in cbox_unrecomms] # 자동접힘 댓글은 못 읽어옴

                    # zip()을 이용하여 content, recomm, unrecomm 합치기
                    comment = list(zip(content, recomm, unrecomm)) # [(c,r,u), ... ]
        ...
        data.to_excel("crawling_data/" + query + "/" + "comments+" + dataTitle + ".xlsx", engine="xlsxwriter")
